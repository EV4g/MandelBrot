...
int[][] mandelGrid(int Mit, int[][] g, boolean sym) {
  int ymax = (sym ? height / 2 + 5 : height);   
...
int[][] rPI(int Mit, int amount, int[][] selection, boolean sym) { //random point iteration
  ...
  while (index < amount) {
    ...
    if (x > 0 && x < width && y > 0 && y < height) {
      if (selection[y][x] != 0) { // == 0 for border, != 0 for all outside of set
        index++;
        Complex c = new Complex(a, b);
        Complex z = new Complex(0, 0);
  ...
  if (sym) {
    for (int x = 0; x < width; x++) {
      for (int y = 0; y < height/2; y++) {
        temp[y + height/2][x] = temp[height/2 - y][x];
      }
    }
  }
...
//1D array version
int[] rPI(int Mit, int amount, int[] selection) { //random point iteration
  int[] temp = new int[height * width];
  double minX = centerX - zoomHorizon/2, minY = centerY - zoomVert/2;
  int index = 0;
